...
client.on("messageCreate", async (msg) => {
    ...
    if (msg.channel.id === config.channel) {
        ...
        let input = "";
        if (attachment) {
            if (msg.attachments[0].contentType.startsWith("image")) {
                input = `Username: ${msg.author.displayName}
Message: ${msg.content}
This user attached an image to their message. The caption is as follows: ${await controller.see(
                    attachment
                )}`;
            } else {
                input = `Username: ${msg.author.displayName}
Message: ${await controller.listen(attachment)}`;
            }
        } else {
            input = `Username: ${msg.author.displayName}
Message: ${msg.content}`;
        ...
        let rsp = await controller.generate(input);
        console.log(rsp);
